...
int	check_borders(t_cub *cube)
{
	...
	x = 0;
	while (cube->map->layout[0][x] == ' ' && x < cube->map->max_line_len)
		x++;
	while (cube->map->layout[0][x] != '\0' && x < cube->map->max_line_len)
	{
...
int	check_textures(char *str) // can open and xmp at the end
{
	...
	while (str[i + 4] != '\0')
		i++;
	if (str[i] != '.' || str[i + 1] != 'x' || str[i + 2] != 'p' || str[i
		+ 3] != 'm')
		return (-1);
...
int	ft_flood_fill(t_cub *cube)
{
	...
	// printf("dadad");
	i = 0;
	rounded_x = cube->pos[0] - 0.5;
	rounded_y = cube->pos[1] - 0.5;
	...
	if (flood_fill_check(layout_copy, rounded_x, rounded_y,
